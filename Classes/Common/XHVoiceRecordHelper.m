...
- (void)prepareRecordingWithPath:(XHPrepareRecorderCompletion)prepareRecorderCompletion {
    WEAKSELF
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (weakSelf) {
            ...
            strongSelf.recordPath = [self getVideoSaveFilePathString];
            error = nil;
...
- (BOOL)createVideoFolderIfNotExist
{
    ...
    NSString *folderPath = [path stringByAppendingPathComponent:VOICE_FOLDER];
...
- (NSString *)getVideoSaveFilePathString
{
    ...
    path = [path stringByAppendingPathComponent:VOICE_FOLDER];
    
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    formatter.dateFormat = @"yyyyMMddHHmmss";
    NSString *nowTimeStr = [formatter stringFromDate:[NSDate dateWithTimeIntervalSinceNow:0]];
    
    NSString *fileName = [[path stringByAppendingPathComponent:nowTimeStr] stringByAppendingString:@".m4a"];
