...
@property (nonatomic, strong) CALayer *videoMask;
...
- (UIImageView*)videoPlayImageView{
    if (_videoPlayImageView == nil) {
        UIImageView *video = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"m_MessageVideoPlay"]];
        video.center = CGPointMake(CGRectGetWidth(self.frame) / 2.0, CGRectGetHeight(self.frame) / 2.0);
        [self addSubview:video];
        _videoPlayImageView = video;
...
- (void)playerVideo:(NSString*)originPhotoUrl onBubbleMessageType:(XHMessageType)bubbleMessageType {
    ...
    [[TRWebDataManager sharedManager] downloadDataWithURL:[NSURL URLWithString:originPhotoUrl] options:TRWebDataRetryFailed&TRWebDataWaitSaveDiskCompletion progress:^(NSInteger receivedSize, NSInteger expectedSize) {
        ...
        STRONGSELF
        if (strongSelf) {
            if (originPhotoUrl && [originPhotoUrl isEqualToString:strongSelf.videoFileURL]) {
...
//    UIGraphicsPushContext(context);
    ...
    CGFloat height = r.size.height+1;//莫名其妙会出现绘制底部有残留 +1像素遮盖
    // 简便起见，这里把圆角半径设置为长和宽平均值的1/10
    CGFloat radius = 6;
    CGFloat margin = 10;//留出上下左右的边距
...
- (void)initAndplayVideoFilePath:(NSString*)filePath{
    ...
    if (_playerLayer == nil) {
        ...
        _videoMask = [self maskOfSize:self.bounds.size];
        _playerLayer.mask = _videoMask;
...
- (void)configureMessageVideo:(NSString *)originPhotoUrl onBubbleMessageType:(XHMessageType)bubbleMessageType {
    ...
    NSString *filePath = [[TRWebDataCache sharedDataCache] diskPathWithKey:originPhotoUrl];
    if (filePath) {
        NSLog(@"configureMessageVideo diskpath %@ ", originPhotoUrl);
        realImage = [XHMessageVideoConverPhotoFactory videoConverPhotoWithVideoPath:filePath];
    }else{
        realImage = [XHMessageVideoConverPhotoFactory videoConverPhotoWithVideoPath:originPhotoUrl];
    }
    if (realImage) {
        self.messagePhoto = realImage;
    }
}
...
- (void)configureMessagePhoto:(UIImage *)messagePhoto thumbnailUrl:(NSString *)thumbnailUrl onBubbleMessageType:(XHMessageType)bubbleMessageType {
    ...
    _videoFileURL =thumbnailUrl;
    NSLog(@"sd_downloadURL %@ size %f, %f ", thumbnailUrl, messagePhoto.size.width, messagePhoto.size.width);
    WEAKSELF
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.messagePhoto) {
        CGSize size = self.messagePhoto.size;
        CGSize drawSize = rect.size;
        CGRect drawRect = rect;

        CGFloat originalR = size.width / size.height;
        CGFloat drawR = drawSize.width /drawSize.height;

        if (originalR < drawR) {
//            drawRect.size.width = drawRect.size.width
            drawRect.size.height = drawSize.width / originalR;
            drawRect.origin.y = - fabs(drawRect.size.height -  drawSize.height) / 2.0;
        }else {
            drawRect.size.width = drawSize.height * originalR;
//            drawRect.size.height = drawRect.size.height;
            drawRect.origin.x = - fabs(drawRect.size.width -  drawSize.width) / 2.0;
        }
        
        [self.messagePhoto drawInRect:drawRect];
    }
    ...
    CGRect blaceRect = CGRectMake(CGRectGetMinX(rect), CGRectGetMinY(rect), CGRectGetWidth(rect), (1 - p) * CGRectGetHeight(rect));
